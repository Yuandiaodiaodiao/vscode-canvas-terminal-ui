...
# 打包 VSIX
npm run package
# 等同于: npx @vscode/vsce package --no-dependencies
```
...
│  ├─ shared-state.js  │              │  ├─ terminal-window.js      │
│  ├─ sync-bridge.js   │              │  ├─ image-window.js         │
│  ├─ terminal-io-     │              │  ├─ browser-window.js       │
│  │   bridge.js       │              │  ├─ sync-handler.js         │
│  └─ terminal-manager │              │  └─ init.js (启动+事件绑定)  │
│      .js (node-pty)  │              └─────────────────────────────┘
└──────────────────────┘
```
...
Sidebar 和独立 Panel 可以同时存在,需要状态同步:

- **shared-state.js** — `SharedStateManager` 持有规范状态 (terminalWindows, imageWindows, config flags),所有状态变更通过它广播
- **sync-bridge.js** — 基于 Named Pipe 的 IPC,广播状态变更到所有 VSCode 实例
- **terminal-io-bridge.js** — 终端 I/O 转发:只有 ownerPid 直接管理 pty,其他实例通过此桥接收输出/发送输入
- **sync-handler.js** (Webview 侧) — 处理 `sync:fullSnapshot`, `sync:windowMoved` 等同步消息
